module Core.Feat.Review.Message.Query.ListNewsletterArticles.Projection.Projection where
...
import Core.Mod.Article.Projection.Newsletters (Newsletters)
...
import Core.Mod.Book.Year.Year as BookYear
import Core.Mod.Editor.Id.Id (EditorId)
import Core.Mod.Editor.Name.Name as EditorName
import Core.Mod.Image.Image (Image)
import Core.Mod.Projection.Finder.Filter (class IsFilter, Contains(..), StrictlyEquals(..), StrictlyNotEquals(..), by, compile)
...
import Core.Mod.Projection.SearchIndex (RawIndexOnly, rawIndexOnly, InvertedIndexOnly, invertedIndexOnly)
...
data ListNewsletterArticlesProjection

instance
  IsProjection
    ListNewsletterArticlesProjection
    "listNewsletterArticles"
    "listNewsletterArticlesProjectionWriteOps"
    (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS ())
    "listNewsletterArticlesProjectionReadSyncProject"
    { article :: Article
    }
  ...
  play = coerce @(LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS ()) play

type LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx = (listNewsletterArticlesProjectionWriteOps :: ProjectionWriteOps | fx)
type LIST_NEWSLETTER_ARTICLES_PROJECTION_READ_SYNC_PROJECT fx = (listNewsletterArticlesProjectionReadSyncProject :: SyncProject | fx)
type LIST_NEWSLETTER_ARTICLES_PROJECTION_READ_FIND fx =
  LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ_FIND
    + fx
...
instance IsPair ArticleKey Article ArticleRecord ArticleIndexNeeds ArticleSortRow "article" "articles" "listNewsletterArticlesArticleProjectionReadFind" ListNewsletterArticlesProjection where
...
type ArticleIndexNeeds =
  ...
  , newsletters ::
      ...
      , ids :: InvertedIndexOnly (Array NewsletterId)
      , issues :: InvertedIndexOnly (Array { month :: Month, year :: TimeYear.Year })
      , mostRecent ::
          { scheduledFor :: RawIndexOnly Instant
          , index :: RawIndexOnly Int
          }
      }
  , illustrations ::
      { hasAtLeastOne :: RawIndexOnly Boolean
...
type ArticleSortRow =
  ...
  , "theme" :: Ɩ
  , "writtenAt" :: Ɩ
  , "newsletters.mostRecent.scheduledFor" :: Ɩ
  , "newsletters.mostRecent.index" :: Ɩ
  , "newsletters.hasAtLeastOne" :: Ɩ
  )
...
instance IsPair AuthorKey Author AuthorRecord AuthorIndexNeeds () "author" "authors" "listNewsletterArticlesAuthorProjectionReadFind" ListNewsletterArticlesProjection where
...
instance IsPair EditorKey Editor EditorRecord EditorIndexNeeds () "editor" "editors" "listNewsletterArticlesEditorProjectionReadFind" ListNewsletterArticlesProjection where
...
instance IsPair BookKey Book BookRecord BookIndexNeeds () "book" "books" "listNewsletterArticlesBookProjectionReadFind" ListNewsletterArticlesProjection where
...
data ArticleFilter
  ...
  | ArticleIsInNewsletterHavingId NewsletterId
  | ArticleIsInNewsletterHavingMonthYear { month :: Month, year :: TimeYear.Year }
  | ArticleIsInScheduledNewsletter
...
instance IsFilter ArticleFilter Article where
  ...
  compile _ ArticleIsInScheduledNewsletter = by @"newsletters.hasAtLeastOne" StrictlyEquals true
  compile _ (ArticleIsInNewsletterHavingId id) = by @"newsletters.ids" Contains [ id ]
  compile _ (ArticleIsInNewsletterHavingMonthYear { month: month, year: year }) = by @"newsletters.issues" Contains [ { month, year } ]
...
type LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ_FIND fx = (listNewsletterArticlesArticleProjectionReadFind :: Find Article | fx)
type LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ fx =
  LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ_FIND
    + LIST_NEWSLETTER_ARTICLES_PROJECTION_READ_SYNC_PROJECT
    + fx

findArticleById :: ∀ fx. ArticleId -> Run (LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ + fx) (Maybe Article)
findArticleById id = findOneByKey (ArticleKey { id: Just id, slug: Nothing })

findArticlesByIds :: ∀ fx. Array ArticleId -> Run (LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ + fx) (Array Article)
findArticlesByIds ids = findManyByKeys (ids <#> \id -> ArticleKey { id: Just id, slug: Nothing })

findArticleByLegacyId :: ∀ fx. Int -> Run (LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ + fx) (Maybe Article)
...
findArticles
  ...
  -> Run (LIST_NEWSLETTER_ARTICLES_ARTICLE_PROJECTION_READ + fx) (Page Article)
findArticles opt = findMany_ opt { after = opt.after <#> (\id -> ArticleKey { id: Just id, slug: Nothing }) ▷ persistenceKeyFromKey }

-- Play 

play :: ∀ fx. LoadedEvent -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
...
onArticleDiscarded :: ∀ fx. ArticleDiscarded.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onArticleDiscarded { article } = delete $ ArticleKey { id: Just article, slug: Nothing }

onArticleWritten :: ∀ fx. Instant -> ArticleWritten.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onArticleWritten
  at
  { id
  , legacyId
  ...
  add $ Article
    ...
    , slug
    , writtenAt: at
    , newsletters: Newsletters.empty
    }

onAuthorReferenced :: ∀ fx. AuthorReferenced.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onAuthorReferenced { id, name, biography, portrait } = add $ Author { id, name, biography, portrait }

onEditorReferenced :: ∀ fx. EditorReferenced.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onEditorReferenced { id, name } = add $ Editor { id, name }

onBookReferenced :: ∀ fx. BookReferenced.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onBookReferenced { id, name, year, cover, authors, editor } = add $ Book { id, name, year, cover, authors, editor }

onNewsletterScheduled :: ∀ fx. NewsletterScheduled.Payload -> Run (LIST_NEWSLETTER_ARTICLES_PROJECTION_WRITE_OPS fx) Ɩ
onNewsletterScheduled { id, scheduledFor, articles } = do
  let
    dt = toDateTime (case scheduledFor of Instant i -> i)
    month = wrap $ Date.month (date dt)
    year = wrap $ Date.year (date dt)

  ø $
    traverse
      ( \(Tuple index articleId) ->
          let
            newItem = { id, index, scheduledFor: { instant: scheduledFor, month, year } }
          in
            patch
              (ArticleKey { id: Just articleId, slug: Nothing })
              ( unwrap
                  ▷
                    ( \a ->
                        let
                        ...
                        in
                          a { newsletters = Newsletters.make (n.newsletters <> [ newItem ]) }
                    )
                  ▷ wrap
              )
      )
      (Array.mapWithIndex Tuple articles)
